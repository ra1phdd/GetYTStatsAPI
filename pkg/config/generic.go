...
func NewLoader(options ...Option) (*Loader, error) {
	...
	configPath := resolveOptionPath(opts.ConfigPath, opts.Resolver.ConfigPath(), opts.Resolver)
	securityPath := resolveOptionPath(opts.SecurityPath, opts.Resolver.SecurityPath(), opts.Resolver)
...
func (l *Loader) LoadInto(target any) error {
	...
	setCredentialResolverBaseDir(filepath.Dir(l.configPath))
...
func (l *Loader) Save(cfg any) error {
	...
	data, err := encodeConfigFile(l.configPath, cfg)
	if err != nil {
...
func resolveOptionPath(path string, fallback string, resolver Replacer) string {
	if strings.TrimSpace(path) == "" {
		return fallback
	}
	return resolver.CleanAbs(path)
}
