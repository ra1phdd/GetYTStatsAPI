// Package credential resolves API credential values for configured API credentials.
//
...
//   - Encrypted:   "enc://<base64>"     → AES-256-GCM decrypt via HUILANBOT_KEY_PASSPHRASE
//   - Empty:       ""                   → returned as-is for providers that do not need a key
//
...
//  2. HUILANBOT_SSH_KEY_PATH env var
//  3. ~/.ssh/huilanbot_ed25519.key (os.UserHomeDir is cross-platform)
package credential
...
const PassphraseEnvVar = "HUILANBOT_KEY_PASSPHRASE"

// PassphraseProvider is the function used to retrieve the passphrase for enc://
// credential decryption. It defaults to reading HUILANBOT_KEY_PASSPHRASE from the
// process environment. Replace it at startup to use a different in-memory source
// when direct env access is not desired.
...
const SSHKeyPathEnvVar = "HUILANBOT_SSH_KEY_PATH"

// projectHomeEnvVar is a package-local copy of config.EnvHome. It is kept here to
// avoid a circular import between pkg/credential and pkg/config.
const projectHomeEnvVar = "HUILANBOT_HOME"

const (
	FileScheme = "file://"
	EncScheme  = "enc://"

	hkdfInfo = "huilanbot-credential-v1"
	saltLen  = 16
	nonceLen = 12
	keyLen   = 32
)

// Resolver resolves raw credential strings for configured api_key fields.
...
//
//   - ""                → "" (no error; some providers need no key)
...
func (r *Resolver) Resolve(raw string) (string, error) {
	...
	if after, ok := strings.CutPrefix(raw, FileScheme); ok {
		...
		if !isWithinDir(realKeyPath, baseDir) {
			return "", errors.New("credential: file:// path escapes config directory")
		}
...
//
// passphrase is required (HUILANBOT_KEY_PASSPHRASE value).
// sshKeyPath is the SSH private key file to use; pass "" to auto-detect via
// HUILANBOT_SSH_KEY_PATH env var or ~/.ssh/huilanbot_ed25519.key.
...
//   - exact match with HUILANBOT_SSH_KEY_PATH env var
//   - within the HUILANBOT_HOME env var directory
//   - within ~/.ssh/
func allowedSSHKeyPath(path string) bool {
	...
	// Exact match with HUILANBOT_SSH_KEY_PATH.
	...
	// Within HUILANBOT_HOME.
	if projectHome := os.Getenv(projectHomeEnvVar); projectHome != "" {
		if isWithinDir(clean, projectHome) {
			return true
		}
	}
...
// Final key: HKDF-SHA256(ikm, salt, info="huilanbot-credential-v1", 32 bytes)
// sshKeyPath must be non-empty; returns an error otherwise.
func deriveKey(passphrase, sshKeyPath string, salt []byte) ([]byte, error) {
	if sshKeyPath == "" {
		return nil, errors.New("credential: SSH private key is required but not found" +
			" (set HUILANBOT_SSH_KEY_PATH or place key at ~/.ssh/huilanbot_ed25519.key)")
	}
	if !allowedSSHKeyPath(sshKeyPath) {
		return nil, fmt.Errorf(
			"credential: SSH key path %q is not in an allowed location (HUILANBOT_SSH_KEY_PATH, HUILANBOT_HOME, or ~/.ssh/)",
			sshKeyPath,
		)
	}
...
//  2. HUILANBOT_SSH_KEY_PATH env var
//  3. ~/.ssh/huilanbot_ed25519.key (auto-detection)
//
...
// findDefaultSSHKey returns the huilanbot-specific SSH key path if it exists.
...
// DefaultSSHKeyPath returns the canonical path for the project SSH key:
// ~/.ssh/huilanbot_ed25519.key.
func DefaultSSHKeyPath() (string, error) {
	home, err := os.UserHomeDir()
	if err != nil {
		return "", fmt.Errorf("credential: cannot determine home directory: %w", err)
	}
	return filepath.Join(home, ".ssh", "huilanbot_ed25519.key"), nil
}
