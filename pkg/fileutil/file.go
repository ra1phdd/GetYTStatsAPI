...
func WriteFileAtomic(path string, data []byte, perm os.FileMode) error {
	...
	// Using CreateTemp avoids collisions under concurrent writes.
...
func renameReplace(src, dst string) error {
	// Unix rename already replaces destination atomically.
	if runtime.GOOS != "windows" {
		return os.Rename(src, dst)
	}

	// Windows rename fails when destination exists and can also briefly return
	// access errors under contention. Retry with best-effort remove.
	var lastErr error
	for range 20 {
